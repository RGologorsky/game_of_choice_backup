...
na_default_folder = "data/full_coop_spe/transitions/NA/c_1.00_b2_1.20_delta_0.9999900000/date_2019_03_04_20_21_20/"


def get_spe_dict(transition, game_str, b1_list, eps):

	# set parent folder
	if   transition == "EqualSay_G2_Default":    folder = g2_default_folder
	elif transition == "EqualSay_G1_Default":    folder = g1_default_folder
	elif transition == "NA":					 folder = na_default_folder
	else: 
		return {round(b1,2): [] for b1 in b1_list}
	...
	for b1 in b1_list:
		...
		if len(spe_lst.shape) == 1:
			spe_lst = np.array([spe_lst])
...
def get_pure_spe_dict(transition, game_str, b1_list):
	...
	for b1 in b1_list:
		...
		spe_lst=[tuple(arr) for arr in spe_lst]  
		#spe_lst = [tuple(eps + (1-2*eps) * x for x in arr) for arr in spe_lst]
