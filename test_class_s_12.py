...
game = S_12_Game(c=1.0, b1=1.8, b2=1.2, game_transition_dynamics="EqualSay_G2_Default")
...
for eps in [0, 0.005, 0.990]:
	...
	print_matrix("Autogen", game.generate_transition_matrix(TFT, WSLS, game.f).tolist())

	assert(np.allclose(TFT_vs_WSLS, game.generate_transition_matrix(TFT, WSLS, game.f)))
	print("Passed transition matrix!")

	# testing stationary distribution
	stationary_TFT_vs_WSLS = game.get_stationary_dist(TFT, WSLS, game.f).tolist()
	stationary_WSLS_vs_TFT = game.get_stationary_dist(WSLS, TFT, game.f).tolist()
